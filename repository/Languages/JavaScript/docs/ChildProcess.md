...
### 参数说明
- command: String, 需要执行的命令, 命令参数与命令之间用空格隔开
- options: Object, 可选参数, 参考:[可选参数option](#关于可选参数option)
- callback: Function, 可选参数, 执行结束后的回调函数, 回调函数参数列表如下:
  - error: Error, 命令执行时发生的异常
  - stdout: Stream, 命令执行时的标准输出
  - stderr: Stream, 命令执行时的异常输出
...
### 参数说明
- file: String, 需要执行的文件
- args: String[], 可选参数, 提交给file文件的执行参数
- options: Object, 可选参数, 参考:[可选参数option](#关于可选参数option)
- callback: Function, 可选参数, 执行结束后的回调函数, 回调函数参数列表如下:
  - error: Error, 命令执行时发生的异常
  - stdout: Stream, 命令执行时的标准输出
...
### 参数说明
- modulePath: String, 子进程需要执行的js模块
- args: String[], 可选参数, 子进程启动时接收的参数列表
- options: Object, 可选参数, 参考:[可选参数option](#关于可选参数option)
...
- cwd: String, 子进程工作目录, 默认值: 主进程的 process.cwd()

- env: Object, 子进程的环境变量配置, 默认值: 主进程的 process.env

- encoding: String, 子进程命令的字符集, 默认值: utf8

- shell
  
  - String, 设置执行命令的Shell, 在Unix系统下默认值: /bin/sh, Windows系统下默认值: process.env.ComSpec
  
  - Boolean, 在部分ChildProcess的API函数中, 该参数为boolean值, 用于设置是否创建Shell来执行命令, 默认值: false
  
  !> 注意: 如果设置为true, 则对于输入的参数需要进行检查, 否则可能存在被注入恶意shell脚本攻击的风险

- signal: Node.Signal, 允许使用的命令中断信号值

- timout: Number, 命令执行的超时时长, 默认值: 0

- maxBuffer: Number, 命令执行时stderr和stdout允许的最大字节数, 默认值: 1024*1024

- killSignal: Node.Signal, 子进程kill信号, 默认值: SIGTERM

- uid: Number, 子进程执行的用户唯一标识

- gid: Number, 子进程执行的用户组唯一标识

- windowsHide: Boolean, 隐藏子进程执行的命令窗口, 通常在Windows系统下会在子进程执行时被操作系统创建, 默认值: false

- windowsVerbatimArguments: Boolean, 在Windows上不需要引用或转义参数, 在Unix系统下忽略此设置, 默认值: false
...
- detached: Boolean, 允许子进程独立于主进程运行, 默认值: false
...
- stdio: String[], 用于设置子进程于主进程见通信的管道, 默认情况下, 子进程的stdin, stdout和stderr被重定向到相应的子进程ChildProcess对象的 subprocess.stdin, subprocess.stdout和subprocess.stderr中, 此时stdio属性的配置值相当于['pipe', 'pipe', 'pipe']即默认值
...
- serialization: "json"|"advanced", 用于设置进程间通信消息的序列化方式, 默认值: "json", 可选值: "json", "advanced"
